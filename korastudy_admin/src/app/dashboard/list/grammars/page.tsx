...
import { grammarsData, role } from "@/lib/data";
...
const GrammarPage = () => {
  ...
  const totalPages = Math.ceil(grammarsData.length / itemsPerPage);

  // Tính toán dữ liệu dựa trên trang hiện tại
  const paginatedData = grammarsData.slice(
    (currentPage - 1) * itemsPerPage,
    currentPage * itemsPerPage
  );

  const renderRow = (item: Grammar) => (
    <tr
      key={item.id}
        ...
        <div className="flex items-center gap-2">
          {role === "admin" && (
            <>
              ...
              <FormModal table="grammar" type="delete" id={item.id} />
            </>
          )}
        </div>
      </td>
